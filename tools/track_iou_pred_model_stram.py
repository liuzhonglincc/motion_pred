...
from group_constrained.Motion_Pred_Group import build as build_motion_pred_model
from group_constrained.tool import load_model
...
def make_parser():
    ...
    parser.add_argument('--weight_t', type=float, default=0.2)
    parser.add_argument('--weight_s', type=float, default=0.2)
    parser.add_argument('--weight_st', type=float, default=0.4)
    return parser
...
def image_track(predictor, vis_folder, args):
    ...
    hidden_dim = 256  # 1024
    d_model = 128   # 256
    model_mp = build_motion_pred_model(args, "MP", d_model, hidden_dim, args.k)
    model_mp.to(device)
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs is not None:
            ...
            trackerTimer.tic()
            online_targets = tracker.update_iou_pred_model_stram(detections, model_mp, device, model)
            trackerTimer.toc()
...
def main(exp, args):
    ...
    return image_track(predictor, "/media/disk1/yjt/lzl/motion_pred/mot_metrix/pred_model", args)
...
if __name__ == "__main__":
    ...
    for seq_num in seqs:
        accs.append(eval_seq(seq_num, "/media/disk1/yjt/lzl/motion_pred/mot_metrix/gt", "/media/disk1/yjt/lzl/motion_pred/mot_metrix/pred_model"))
    ...
    fw = open(f"/media/disk1/yjt/lzl/motion_pred/test.txt", mode="a+")
    fw.write(f"match_thresh: {args.match_thresh}\n")
    fw.write(strsummary)
    fw.write("\n")
    fw.close()
